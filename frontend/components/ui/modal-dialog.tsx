...
export function ModalDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#090909]/78 p-4 backdrop-blur-xl">
      <motion.section
        initial={{ opacity: 0, y: 14, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.18, ease: [0.16, 1, 0.3, 1] }}
        className={`crm-popover-surface max-h-[92vh] w-full overflow-y-auto p-5 ${maxWidthClassName}`}
        role="dialog"
        aria-modal="true"
        aria-label={title}
      >
